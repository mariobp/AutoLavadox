create or replace function informe_time_operarios(f1 text,f2 text) returns json as $$
declare
 tipos_servi json;
 empleados json;
begin
          	empleados:=(SELECT COALESCE(array_to_json(array_agg(row_to_json(p2))), '[]') from (
                ...
                from operacion_tiposervicio t
                      ...
                      left join operacion_orden as o on (s.orden_id=o.id and o.activo=true and o.pago=true)
