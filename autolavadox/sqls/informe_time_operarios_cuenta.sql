-- Function: public.informe_time_operarios_cuenta(text, text, integer)

-- DROP FUNCTION public.informe_time_operarios_cuenta(text, text, integer);

CREATE OR REPLACE FUNCTION public.informe_time_operarios_cuenta(
    f1 text,
    f2 text,
    id_cuenta integer)
  RETURNS json AS
$BODY$
		 	...
		 	empleados:=(SELECT COALESCE(array_to_json(array_agg(row_to_json(p2))), '[]') from (
		       ...
		       case when s.inicio is not null and s.fin is not null then
		         ...
		         from operacion_tiposervicio t
		             ...
		             left join operacion_orden as o on (o.cuenta_id=id_cuenta and s.orden_id=o.id and o.activo=true and o.pago=true and cast(o.fin as date)>= cast(f1 as date) and cast(o.fin as date)<= cast(f2 as date))
				 ...
				 inner join empleados_persona as p on (e.persona_ptr_id=p.user_ptr_id and p.cuenta_id=id_cuenta)
		         inner join auth_user as u on (u.id=p.user_ptr_id)
		      ) p2);
		      tipos_servi := (SELECT COALESCE(array_to_json(array_agg(row_to_json(p22))), '[]') from (
				select nombre from operacion_tiposervicio where cuenta_id=id_cuenta order by id asc
		      ) p22);
  ...
  COST 100;
ALTER FUNCTION public.informe_time_operarios_cuenta(text, text, integer)
  OWNER TO postgres;
