-- Function: public.cierre_total_tipo_factura_cuenta(text, text, integer)

-- DROP FUNCTION public.cierre_total_tipo_factura_cuenta(text, text, integer);

CREATE OR REPLACE FUNCTION public.cierre_total_tipo_factura_cuenta(
    d1 text,
    d2 text,
    id_cuenta integer)
  RETURNS json AS
$BODY$
		                    ...
		                    from (select  ts.id,ts.nombre,ts.costo,ts.comision,
		                        ...
		                        inner join operacion_servicio as s on
		                                (s.orden_id=o.id and s.status=true and s.estado=true
		                                 ...
		                                 and o.activo=true and o.cerrada=true and o.pago=true and o.cuenta_id=id_cuenta)
  ...
  COST 100;
ALTER FUNCTION public.cierre_total_tipo_factura_cuenta(text, text, integer)
  OWNER TO postgres;
