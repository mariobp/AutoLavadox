-- Function: public.cierre_factura_total_cuenta(text, text, integer)

-- DROP FUNCTION public.cierre_factura_total_cuenta(text, text, integer);

CREATE OR REPLACE FUNCTION public.cierre_factura_total_cuenta(
    d1 text,
    d2 text,
    id_cuenta integer)
  RETURNS json AS
$BODY$
  ...
  COST 100;
ALTER FUNCTION public.cierre_factura_total_cuenta(text, text, integer)
  OWNER TO postgres;
