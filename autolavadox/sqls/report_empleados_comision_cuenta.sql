-- Function: public.report_empleados_comision_cuenta(text, text, integer)

-- DROP FUNCTION public.report_empleados_comision_cuenta(text, text, integer);

CREATE OR REPLACE FUNCTION public.report_empleados_comision_cuenta(
    d1 text,
    d2 text,
    id_cuenta integer)
  RETURNS json AS
                            ...
                            from
                                (
                                   ...
                                   case when s.id is not null and ser_ope.empleado_id is not null then (select count(id)from operacion_servicio_operario as oper where oper.servicio_id=s.id) else 0 end as ope_t
                                              ...
                                              left join operacion_servicio as s on (o.cuenta_id=id_cuenta and s.orden_id=o.id and o.pago=true and cast(o.fin as date)>= cast(d1 as date) and cast(o.fin as date)<= cast(d2 as date))
...
$BODY$
  ...
  COST 100;
ALTER FUNCTION public.report_empleados_comision_cuenta(text, text, integer)
  OWNER TO postgres;
